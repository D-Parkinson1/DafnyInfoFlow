...
namespace Microsoft.Dafny {
  ...
  public class Main {
    ...
    public static string Parse(IList<DafnyFile> files, string programName, ErrorReporter reporter, out Program program) {
      ...
      if (DafnyOptions.O.ifInit) {
        var injector = new DafnyPipeline.InformationFlowVisitor(program);
        injector.Init(addAssertions: DafnyOptions.O.ifAssert);
        string outFile;
        if (DafnyOptions.O.ifAssert) {
          // Print output to stdin when adding assertions
          outFile = "-";
        } else {
          // Print output to .if.dfy file, to update it.
          var inFilePath = files[0].FilePath;
          if (inFilePath.EndsWith(".if.dfy")) {
            outFile = inFilePath;
          } else {
            outFile = inFilePath.Insert(inFilePath.Length - 4, ".if");
          }
        }
        // In case user is printing a file, and doesn't want everything printed
        var givenPrintMode = DafnyOptions.O.PrintMode;
        DafnyOptions.O.PrintMode = DafnyOptions.PrintModes.Everything;
        MaybePrintProgram(program, outFile, false);
        DafnyOptions.O.PrintMode = givenPrintMode;
      }
